...
function search_news(part)
{
    var part='tin-tuc/tim-kiem/';
	var key = $('#keysearch').val();
	if(key == '')
		{
			alert('B?n chưa nh?p t? khóa.');
			$('#keysearch').focus();
			
		}
	else window.location.href = base_url + part+'/' + encodeURIComponent(key);
...
function search_news(f)
{
	...
	if(key == '' || key == 'T?m trong tin t?c')
		{
			alert('B?n chưa nh?p t? khóa.');
			jQuery('#keysearch').focus();
			
		}
	else window.location.href = base_url + 'tin-tuc/tim-kiem/' + encodeURIComponent(key);
